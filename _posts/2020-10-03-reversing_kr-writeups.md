...
![1]({{ '/assets/img/posts/reversing_kr/easy_crack.png' | prepend: site.url }})
...
![2]({{ '/assets/img/posts/reversing_kr/easy_keygen.png' | prepend: site.url }})
...
![3]({{ '/assets/img/posts/reversing_kr/music_player.png' | prepend: site.url }})
...
1. 破解第一个弹窗
    ...
    ![4]({{ '/assets/img/posts/reversing_kr/music_player_1.png' | prepend: site.url }})

    ![5]({{ '/assets/img/posts/reversing_kr/music_player_2.png' | prepend: site.url }})

    此外在上面找到跳转函数 `JL Music_Pl.004045FE`，因为我们需要将程序流程跳过该弹窗，因此修改指令为`jmp 004045FE` 无条件跳转。
    之后选择修改区域，右键 `复制到可执行文件` 后保存文件，运行后出现第二个弹窗

    ![6]({{ '/assets/img/posts/reversing_kr/music_player_3.png' | prepend: site.url }})
